  ...
  try {
    ...
    const categories = await Category.create([
      ...
      },
      {
        ...
        description: 'Bí kíp cài đặt AI Agents từ A-Z, quy trình phân quyền, tối ưu hóa token và thiết lập chỉ dẫn vàng.',
        icon: 'RobotOutlined',
        color: '#8B5CF6',
        order: 17
      }
...
- Quy định tiếp theo dành cho Agent...`
  ...
  "mcpServers": {
    ...
    "custom-remote-server": {
      ...
      // --- AI Agent Playbook ---
      {
        categoryId: catMap['ai-agent-playbook'],
        title: 'Claude Code - Cài đặt & Phân quyền bảo mật',
        slug: 'claude-code-setup',
        command: 'npm install -g @anthropic-ai/claude-code && claude',
        description: 'Anthropic Claude Code là AI Agent tự trị chạy trực tiếp trong terminal của bạn. Nó có quyền đọc, ghi file và thực thi lệnh hệ thống. Thẻ này hướng dẫn cài đặt và thiết lập bảo mật.',
        explanations: [
          { param: 'npm install -g @anthropic-ai/claude-code', description: 'Cài đặt global công cụ Claude Code CLI chính thức từ Anthropic.' },
          { param: 'claude', description: 'Khởi chạy giao diện dòng lệnh của Claude Code để bắt đầu tương tác.' }
        ],
        examples: [
          { title: 'Chạy chế độ chỉ đọc (Safe Mode)', command: 'claude --read-only', description: 'Khởi chạy Claude Code ở chế độ an toàn, chỉ cho phép đọc code và phân tích lỗi mà không có quyền chỉnh sửa file hay chạy lệnh terminal.' }
        ],
        workflows: [
          {
            description: 'Khởi chạy và Xác thực tài khoản lần đầu',
            content: 'claude',
            terminalScript: [
              '$ npm install -g @anthropic-ai/claude-code',
              '✓ Installed @anthropic-ai/claude-code@0.1.5 successfully.',
              '$ claude',
              'Welcome to Claude Code (beta)!',
              'This tool has access to your local filesystem and can execute terminal commands.',
              'Please complete OAuth authentication in your browser to log in to Anthropic...',
              '✓ Successfully authenticated as Nguyen Van Thiet.',
              'Session key saved. Type /help to see all available slash commands.',
              'claude-code (b50467c6) > '
            ]
          }
        ],
        tags: ['ai', 'claude', 'setup', 'security', 'autonomous'],
        viewCount: 720
      },
      {
        categoryId: catMap['ai-agent-playbook'],
        title: 'Claude Code - Tối ưu hóa chi phí & Tiết kiệm Token',
        slug: 'claude-code-token-save',
        command: 'claude --ignore-files ".git,node_modules,dist,build"',
        description: 'Mặc định, Claude Code sẽ tự động lập chỉ mục (index) và đọc rất nhiều file để nắm bắt ngữ cảnh dự án. Nếu không chặn các thư mục nặng như node_modules hay thư mục build, bạn sẽ bị vọt chi phí API lên tới hàng chục USD chỉ trong vài phút!',
        explanations: [
          { param: '--ignore-files', description: 'Danh sách các thư mục hoặc file viết cách nhau bởi dấu phẩy mà Claude không được phép đọc.' }
        ],
        examples: [
          { title: 'Tạo file bỏ qua tập trung', command: 'echo "node_modules/\\ndist/\\n.git/\\n*.log" > .claudeignore', description: 'Tạo file .claudeignore ở thư mục gốc dự án để Claude tự động bỏ qua vĩnh viễn các thư mục rác khi phân tích.' }
        ],
        workflows: [
          {
            description: 'Khởi chạy với cấu hình bỏ quan nâng cao',
            content: 'claude --ignore-files "node_modules,dist,.git,*.png,*.jpg"',
            terminalScript: [
              '$ claude --ignore-files "node_modules,dist,.git,*.png,*.jpg"',
              'ℹ Initializing workspace context...',
              'ℹ Loaded 12 custom rules from .clauderules',
              '✓ Ignored 4,500 files inside matching ignore-patterns (node_modules, dist).',
              '✓ Context minimized successfully. Active token usage: 840 tokens (Saved 98% cost!).',
              'claude-code (token-optimized) > '
            ]
          }
        ],
        tags: ['ai', 'claude', 'token', 'cost-save', 'optimize'],
        viewCount: 650
      },
      {
        categoryId: catMap['ai-agent-playbook'],
        title: 'Aider AI - Lập trình cặp nâng cao (Git Integration)',
        slug: 'aider-setup',
        command: 'pip install aider-chat && aider',
        description: 'Aider là AI Coding Agent lập trình cặp qua CLI tốt nhất hiện tại. Aider kết nối trực tiếp với local Git repository của bạn để tự động phân tích thay đổi và tạo git commit có ý nghĩa chuẩn xác sau mỗi dòng code chỉnh sửa thành công.',
        explanations: [
          { param: 'pip install aider-chat', description: 'Cài đặt thư viện Aider qua trình quản lý gói Python.' },
          { param: 'aider', description: 'Khởi chạy Aider trong thư mục Git hiện tại.' }
        ],
        examples: [
          { title: 'Sử dụng model chuyên dụng Claude 3.5 Sonnet', command: 'aider --model claude-3-5-sonnet', description: 'Yêu cầu Aider sử dụng mô hình tối ưu nhất cho lập trình của Anthropic.' }
        ],
        workflows: [
          {
            description: 'Quy trình tương tác chuẩn chỉnh giúp kiểm soát ngữ cảnh',
            content: 'aider',
            terminalScript: [
              '$ aider',
              'Aider v0.32.0 - CLI Pair Programming',
              'Using Claude 3.5 Sonnet...',
              'Git repository detected. Aider is ready to write commits automatically.',
              'Use /add to load files into memory, or /drop to release them.',
              'aider-chat > /add src/components/Header.tsx',
              'Added src/components/Header.tsx to the chat context.',
              'aider-chat > Sửa kích thước QR code ngân hàng to lên một chút.',
              'Thinking...',
              'Applied changes to src/components/Header.tsx successfully.',
              'Running git diff...',
              '✓ Created git commit: "style(Header): increase QR code bank card frame size"',
              'aider-chat > '
            ]
          }
        ],
        tags: ['ai', 'aider', 'git', 'pair-programming', 'python'],
        viewCount: 580
      },
      {
        categoryId: catMap['ai-agent-playbook'],
        title: 'Antigravity & Cursor - Chỉ thị cấu hình Rules Vàng',
        slug: 'ai-golden-rules',
        command: 'touch .antigravityrules',
        description: 'File .antigravityrules (hoặc .cursorrules) đặt ở thư mục gốc của dự án chứa các "bộ luật chỉ dẫn" bắt buộc AI phải tuân thủ tuyệt đối (ví dụ: cấm dùng any, cấm viết code ẩu, bắt buộc viết test...). Thẻ này cung cấp cẩm nang Rules vàng giúp AI tăng năng suất code lên 200%.',
        explanations: [],
        examples: [
          { title: 'Tạo file Rules vàng', command: 'touch .antigravityrules && code .antigravityrules', description: 'Tạo file cấu hình và mở trên trình soạn thảo code của bạn.' }
        ],
        workflows: [
          {
            description: 'Cấu trúc chỉ dẫn tối ưu cho Dự án React/TypeScript',
            content: 'cat .antigravityrules',
            terminalScript: [
              '# .antigravityrules - Golden Instructions',
              '## Core Principles',
              '- Prefer simple, single-responsibility functions over complex architecture.',
              '- Never use the "any" type. Strictly enforce type safety with detailed interfaces.',
              '- Keep layout CSS flexible using rem, grid/flex, and custom HSL token variables.',
              '- Always run test coverage and static lints before announcing completion.',
              '## Coding Style',
              '- Do not write obvious comments that duplicate what the clean code already expresses.',
              '- Ensure hydration safety in Next.js Server Components. Avoid waterfalls.',
              '✓ Configured! AI Agent will now automatically obey this system prompt.'
            ]
          }
        ],
        tags: ['rules', 'cursor', 'antigravity', 'config', 'golden-prompt'],
        viewCount: 940
      }
    ]);
